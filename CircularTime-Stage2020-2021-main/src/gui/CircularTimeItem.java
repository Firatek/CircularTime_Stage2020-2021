...
public class CircularTimeItem{
	...
	public CircularTimeItem(Ellipse2D.Double max, Ellipse2D.Double min, double angleMin, double angleMax, double val, int rank, int formatLayer){
		Arc2D.Double arc = new Arc2D.Double(max.getBounds2D(),angleMin,angleMax,Arc2D.PIE);
		item = new Area(arc);
	...
	public void paint(Graphics2D g2){
		...
		cal.set(Calendar.MONTH, rank);
		String month = cal.getDisplayName(Calendar.MONTH, Calendar.LONG, Locale.getDefault());
		afficheTexte(g2, month);
		switch (state) {
		case SELECT:
	...
	public void afficheTexte(Graphics2D g2, String texte){
        ...
        Rectangle2D box = item.getBounds2D();
